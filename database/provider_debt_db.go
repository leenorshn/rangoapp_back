...
func (db *DB) CreateProviderDebt(
	...
	if err != nil {
		return nil, gqlerror.Errorf("Error creating provider debt: %v", err)
	}
...
func (db *DB) PayProviderDebt(
	...
	if err != nil {
		return nil, nil, gqlerror.Errorf("Invalid provider debt ID")
	}
	...
	if err != nil {
		if err == mongo.ErrNoDocuments {
			return nil, nil, gqlerror.Errorf("Provider debt not found")
		}
		return nil, nil, gqlerror.Errorf("Error finding provider debt: %v", err)
	}
	...
	if debt.StoreID != storeID {
		return nil, nil, gqlerror.Errorf("Provider debt does not belong to the specified store")
	}
	...
	if err != nil {
		return nil, nil, gqlerror.Errorf("Error updating provider debt: %v", err)
	}

	// Reload debt
	err = debtCollection.FindOne(ctx, bson.M{"_id": objectID}).Decode(&debt)
	if err != nil {
		return nil, nil, gqlerror.Errorf("Error reloading provider debt: %v", err)
	}
...
func (db *DB) GetProviderDebtByID(debtID string) (*ProviderDebt, error) {
	objectID, err := primitive.ObjectIDFromHex(debtID)
	if err != nil {
		return nil, gqlerror.Errorf("Invalid provider debt ID")
	}
	...
	if err != nil {
		if err == mongo.ErrNoDocuments {
			return nil, gqlerror.Errorf("Provider debt not found")
		}
		return nil, gqlerror.Errorf("Error finding provider debt: %v", err)
	}
...
func (db *DB) GetProviderDebtBySupplyID(supplyID string) (*ProviderDebt, error) {
	...
	if err != nil {
		return nil, gqlerror.Errorf("Invalid supply ID")
	}
...
func (db *DB) GetProviderDebtsByProviderID(providerID string, storeID *string) ([]*ProviderDebt, error) {
	...
	if err != nil {
		return nil, gqlerror.Errorf("Error finding provider debts: %v", err)
	}
	defer cursor.Close(ctx)

	var debts []*ProviderDebt
	if err = cursor.All(ctx, &debts); err != nil {
		return nil, gqlerror.Errorf("Error decoding provider debts: %v", err)
	}
